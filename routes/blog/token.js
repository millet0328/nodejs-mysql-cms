...
/**
 * 路由"/token/refresh/blog"----单独校验
 * 校验refresh token是否有效，无效则抛出Error/UnauthorizedError
 */

router.use("/refresh/blog", token_manager.guard([], async (req, token) => {
    ...
    // 检测refresh_token是否失效
    let select_sql = 'SELECT user_id, refresh_token FROM `cms_user` WHERE jwt_id = ?';
 ...
 * 返回403状态码,返回错误信息
 */
router.use('/refresh/blog', (err, req, res, next) => {
