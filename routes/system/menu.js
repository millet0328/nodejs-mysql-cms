...
router.put("/:menu_id", async (req, res) => {
    ...
    try {
        ...
        await connection.beginTransaction();
        // 更新 menu 路由
        let update_menu_sql = 'UPDATE `sys_menu` SET menu_name = ?, route_id = ?, menu_order = ?, icon_id = ? WHERE menu_id = ?';
        let [{ affectedRows: menu_affected_rows }] = await pool.query(update_menu_sql, [menu_name, route_id, menu_order, icon_id, menu_id]);
        if (menu_affected_rows === 0) {
            await connection.rollback();
        ...
        // 更新 permission 权限
        let update_permission_sql = 'UPDATE `sys_permission` SET parent_id = ?, permission_code = ?, permission_remark = ? WHERE resource_id = ? AND resource_type_id = 2';
        let [{ affectedRows: permission_affected_rows }] = await pool.query(update_permission_sql, [parent_id, route_full_path, permission_remark, menu_id]);
        if (permission_affected_rows === 0) {
            await connection.rollback();
...
/**
 * @api {put} /menu/icon/:menu_id 设置菜单图标
...
/**
 * @api {post} /menu/operation 给菜单页面添加新的操作按钮
